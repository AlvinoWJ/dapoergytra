...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    if (!checked || cartLoading) return;

    if (items.length === 0 && !isCheckingOut) {
      router.replace("/dashboard");
    }
  }, [checked, items.length, isCheckingOut, router, cartLoading]);

  const isLoading = !checked || !isLoggedIn || cartLoading;
